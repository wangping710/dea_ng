...
describe Dea::Snapshot do
  ...
  describe "load  the starting   snapshot" do
    before do
      File.open(snapshot.path, "w") do |file|
        saved_snapshot = {
            "instances" => [
    ...
    it "should load a snapshot,instance change to crahsed" do
        instance = Dea::Instance.new(bootstrap, valid_instance_attributes)
        instance.stub(:validate)
  ...
  describe "load  the running state snapshot" do
    before do
      File.open(snapshot.path, "w") do |file|
        saved_snapshot = {
            "instances" => [
    ...
    it "should load a snapshot,instance change to running" do
        instance = Dea::Instance.new(bootstrap, valid_instance_attributes)
        instance.stub(:validate)
